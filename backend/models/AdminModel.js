...
const addEvent = async (eventData) => {
  ...
  // Ensure the correct number of values are being inserted
  const query = `
  INSERT INTO events (
    from_time, to_time, event_date, day, venue, event_name,
    event_description, event_coordinator_name, event_coordinator_phone,
...
// Function to add event files
const addEventFiles = async (fileDetails) => {
  const query = `
    INSERT INTO event_files (event_id, file_name, file_path, file_type)
    VALUES ?
  `;

  const values = fileDetails.map((file) => [file.eventId, file.fileName, file.filePath, file.fileType]);

  try {
    await pool.query(query, [values]);
  } catch (error) {
    console.error("Error inserting event files into database:", error);
    throw error;
  }
};
